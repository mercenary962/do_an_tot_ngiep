...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        ...
        <input id="inputTitle" type="text" name="type" placeholder="Nhập loại"  value="{{$shipping->type}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="inactive" {{(($shipping->status=='inactive') ? 'selected' : '')}}>Vô hiệu hóa</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
